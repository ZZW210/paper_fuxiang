...
- `stage2_final_conflict_points`: 32
- `final_Tdelay`: 25188.771991324717
- `final_Tair`: 59789.435065154146
- `final_ORISK`: 1131.7592156502678
- `final_n_delay`: 0
- `final_n_battery`: 0
- `final_paper_fitness`: 0.3380871944076152
- `stage1_fata_time`: 47.1818226000014
- `stage2_fata_time`: 71.98792879999928
- `total_runtime`: 125.50402949999989
...
- `python -m pytest -q`：32 passed，含真正ADM解码的n_jobs=1与8一致性测试，以及主流程HTML/CSV/Markdown输出测试。
- `python check_paper_strict.py`：24项PASS，返回码0。
- strict quick，seed=2025，100条计划，NP=20，两阶段各50代：冲突点130 → 99 → 56。
- strict normal，seed=2025，100条计划，NP=50，两阶段各200代：冲突点130 → 98 → 32。
- 重新载入最终PKL并重新探测，确认最终32个冲突点；重新计算fitness=0.3380871944076152，与metrics_summary.csv一致，所有原始目标分量也一致。
- 收敛CSV包含完整400代，阶段边界200/201；ADM有98个独立冲突点单位、含第0代的19698行历史，所有概率行之和为1。
- 全部最终计划的speed_profile长度等于路径航段数量。最终n_delay=0、n_battery=0；没有调用额外repair来补零冲突。
